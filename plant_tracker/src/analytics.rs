...
fn find_indx_after_watering(plant: &Plant) -> Vec<usize> {
    plant.measurements.iter()
    .enumerate()
    .filter(| (_, m)| m.type_ == MeasurementType::AfterWatering || m.type_ == MeasurementType::AfterWateringWithFeed)
    .map(|(i,_)| i)
    .collect()
...
fn calculate_r_per_cycle(plant: &Plant, slice: &[[usize; 2]]) -> Vec<f32> {
    ...
    .flat_map(|[start, end]| {
        ...
        let regulars: Vec<&Measurement> = cycle.iter()
        .filter(|x| x.type_ == MeasurementType::Regular)
        .collect();

        if regulars.len() < 2 {return None;}
...
fn get_current_cycle_r(plant: &Plant) -> Option<f32> {
    let last_watering_idx = plant.measurements
    .iter()
   
    .rposition(| t| t.type_ == MeasurementType::AfterWatering || t.type_ == MeasurementType::AfterWateringWithFeed)?;
