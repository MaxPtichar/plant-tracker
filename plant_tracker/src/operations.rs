...
pub async fn transpiration_coef_calc(
    ...
    let last_regular_before =
        db_operations::get_last_regular_before_watering(pool, plant_id).await?;
    ...
    let new_threshold = match last_regular_before {
        Some(m) => {
            let actual_threshold =
                ((m.weight - dry_total) / (last_watered - dry_total)).clamp(0.05, 0.95);
            (0.7 * old_threshold + 0.3 * actual_threshold).clamp(min_threshold, 0.95)
        }
        None => *old_threshold,
    };
